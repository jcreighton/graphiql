...
var QueryHistory = (function (_super) {
  ...
  function QueryHistory(props) {
    ...
    _this.updateHistory = function (query, variables, headers, operationName) {
      if (
        shouldSaveQuery(
          ...
          variables,
          headers,
          _this.historyStore.fetchRecent(),
        )
      ) {
    ...
    _this.toggleFavorite = function (
      ...
      _this.setState({
        queries: __spreadArrays(
          _this.historyStore.items,
          _this.favoriteStore.items,
        ),
      });
    };
    _this.editLabel = function (
      query,
      variables,
      headers,
      operationName,
      ...
      if (favorite) {
        _this.favoriteStore.edit(
          __assign(__assign({}, item), { favorite: favorite }),
        );
      } else {
    ...
    _this.historyStore = new QueryStore_1.default(
      'queries',
      props.storage,
      MAX_HISTORY_LENGTH,
    );
    _this.favoriteStore = new QueryStore_1.default(
      'favorites',
      props.storage,
      null,
    );
